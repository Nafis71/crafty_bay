...
class AuthViewModel extends GetxController{
  ...
  Future<bool> verifyOTP(String emailAddress,String otp) async{
    ...
    if(response is Success){
      ...
      bool status = await checkUserData(response!, token);
      _hasUserData = status;
  ...
  Future<bool> checkUserData(Object response, String token) async{
    response = await AuthService().readProfile(token);
    if(response is Success){
      Map<String,dynamic> jsonData = (response).response as Map<String,dynamic>;
