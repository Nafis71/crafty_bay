...
import 'package:crafty_bay/features/onboarding/splash_view/utils/splash_view_helper.dart';
...
class _SplashViewState extends State<SplashView> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: GetBuilder<SplashViewState>(builder: (splashViewState) {
        return GetBuilder<NetworkState>(builder: (networkState) {
          if (networkState.hasInternet) {
            SplashViewHelper.prefetchData(
              splashViewState: splashViewState,
              context: context,
            );
          } else {
            if (!splashViewState.hasShownSnackBar) {
              splashViewState.setHasShownSnackBar = true;
              Future.delayed(
